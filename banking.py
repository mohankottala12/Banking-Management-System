...
def account_status(acno):
    conn = mysql.connector.connect(
        host = 'localhost', database='bankproject', user='root', password='toor')
    cursor = conn.cursor()
...
def check_balance():
    ...
    result = account_status(accno)
    if result [0]== 'active':
        sql1 = 'select balance from customer where '+accno+' = acno;'
        cursor.execute(sql1)
        record = cursor.fetchone()
        print('Account Balance : ',record[0])
    else:
